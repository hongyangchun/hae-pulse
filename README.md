...
| 静息心率 | 最近一次静息心率 + 7 日均值。**日结型指标，清晨前当天还没有**，这时右列会带数据日期（如 ` · 09-16`）；当天的点落库后就不带 |
...
**静息心率/睡眠/体重/心肺耐力这几行右边可能带一个 ` · MM-DD`**，那是数据的日期。它们都是
**日结型**指标：由整夜的数据算出，Apple 清晨才定稿。所以清晨之前最新点还是昨天的（会带日期），
清晨之后当天的点就出现了（不带日期）—— 两种都会遇到，不标日期就可能把昨天的读数读成今天的。
规则在 `collector.py` 的 `latest()` 与 `render.py` 的 `day_suffix()` / `Main.qml` 的 `daySuffix()` —— 三端一致。

**「锻炼」用的是今日训练时长，不是 Apple 锻炼环。** 锻炼环（`apple_exercise_time`）是日结型，
清晨前当天的值拿不到，拿它算「今日锻炼」会恒为 0；`/api/workouts` 是当天实时写入的，只有它当天会动。
...
- **日结型指标别只取「今天」**：静息心率/睡眠/体重/心肺耐力/锻炼环由整夜数据算出，
  清晨前当天的点还没落库，只取今天会恒为空。取「最后一个非空值」+ 按日期决定标不标。
  判断某指标属于哪类最快的入口是 `/api/metrics` 的 `last_day`（= 今天说明当天的点已有，
  = 昨天说明还没落库）—— 注意它每天会变，别当成固定属性。
...
- **但「信息面」不等于「文案面」**：Omarchy 一个 `Main.qml` 里就有 3 份文案（bar / tooltip / popout），
  加上 macOS 下拉共 4 处。**同一个量出现在几处，就有几处要改** —— 行序、口径、日期规则、
  数字格式四样必须逐处对齐。工具：`node scripts/verify_surface_parity.mjs`（无依赖、不联网，可进 pre-commit），
  它做结构对拍而不是复制渲染逻辑，所以不会自己跟着漂。

## 验证

```bash
node scripts/verify_surface_parity.mjs   # 文案面一致性：行序 / 日期规则 / 千分位 / 两端字段口径
python3 -m py_compile collector.py macos/render.py
```
