...
行序与 Omarchy 弹层、网页仪表盘**三端一致**：
`HRV → 静息心率 → 心肺耐力(估) → 睡眠 → 体重 → 锻炼`。

**面板**：点「打开面板」打开 440×620 的弹层，即 Omarchy 版那个 panel 的等价物。
定位是**降级保留** —— 只放下拉放不下的东西（趋势图 + 跳转完整仪表盘的入口），
不做下拉的放大版；主题跟随系统深浅色。
...
panel.template.html      面板模板（跟随系统深浅色），数据由 render.py 内联写入
...
2. **配色双份**。原版直接用 `#8FF740` / `#d7a55b` 这套深色主题色。macOS 菜单栏
   ...
   **面板也跟随系统浅/深色**（`prefers-color-scheme`），不再恒暗 —— 恒暗时浅色 Mac 上
   弹层是黑的、下拉是浅的，两边不像一家人。
